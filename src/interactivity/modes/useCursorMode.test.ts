...
import { InteractivityActionType, InteractivityMode, InteractivityTarget, CursorConfig } from "../types";
...
const fireMoveAt = (x: number, y: number) => {
  ...
  vi.spyOn(container, "getBoundingClientRect").mockReturnValue({
    ...
    right: 100, bottom: 100, x: 0, y: 0, toJSON: vi.fn(),
  });
...
describe("useCursorMode", () => {
  it("does not attach listener when disabled", () => {
    renderHook(() => { useCursorMode(target, SEEK_CONFIG, false); });
    fireMoveAt(50, 50);
    act(() => { vi.runAllTimers(); });
  ...
  it("calls goToAndStop after mousemove fires and rAF runs (seek action)", () => {
    renderHook(() => { useCursorMode(target, SEEK_CONFIG, true); });
    fireMoveAt(50, 50);
    act(() => { vi.runAllTimers(); });
  ...
  it("calls play() when cursor enters a play-action zone", () => {
    const playConfig: CursorConfig = {
      mode: InteractivityMode.cursor,
      actions: [{ position: { x: [0, 1] }, type: InteractivityActionType.play }],
    };
    // Simulate animation is paused
    animItem.isPaused = true;
    renderHook(() => { useCursorMode(target, playConfig, true); });
    fireMoveAt(50, 50);
    act(() => { vi.runAllTimers(); });
  ...
  it("calls stop() when cursor is in a stop-action zone and animation is playing", () => {
    const stopConfig: CursorConfig = {
      mode: InteractivityMode.cursor,
      actions: [{ position: { x: [0, 1] }, type: InteractivityActionType.stop }],
    };
    animItem.isPaused = false;
    renderHook(() => { useCursorMode(target, stopConfig, true); });
    fireMoveAt(50, 50);
    act(() => { vi.runAllTimers(); });
  ...
  it("removes listeners on unmount", () => {
    const { unmount } = renderHook(() => { useCursorMode(target, SEEK_CONFIG, true); });
    unmount();
    fireMoveAt(50, 50);
    act(() => { vi.runAllTimers(); });
    expect(animItem.goToAndStop).not.toHaveBeenCalled();
  ...
  it("sets loop=true and plays when cursor enters a loop-action zone", () => {
    const loopConfig: CursorConfig = {
      mode: InteractivityMode.cursor,
      actions: [{ position: { x: [0, 1] }, type: InteractivityActionType.loop }],
    };
    animItem.isPaused = true;
    renderHook(() => { useCursorMode(target, loopConfig, true); });
    fireMoveAt(50, 50);
    act(() => { vi.runAllTimers(); });
