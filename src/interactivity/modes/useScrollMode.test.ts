...
import { InteractivityActionType, InteractivityMode, InteractivityTarget, ScrollConfig } from "../types";
...
describe("useScrollMode", () => {
  ...
  it("adds scroll listener to window when element intersects", () => {
    ...
    act(() => { observer.trigger(container, true); });
  ...
  it("calls goToAndStop when scroll fires (seek action)", () => {
    ...
    vi.spyOn(container, "getBoundingClientRect").mockReturnValue({
      top: 500, bottom: 600, height: 100, width: 200, left: 0, right: 200,
      x: 0, y: 500, toJSON: vi.fn(),
    });
    ...
    const observer = MockIntersectionObserver.getLastOrFail();
    act(() => { observer.trigger(container, true); }); // adds scroll listener + calls handleScroll
  ...
  it("disconnects observer and removes scroll listener on unmount", () => {
    const { unmount } = renderHook(() => { useScrollMode(target, SEEK_CONFIG, true); });
  ...
  it("calls play() when element intersects with play action and animation is paused", () => {
    ...
    renderHook(() => { useScrollMode(target, playConfig, true); });
    const observer = MockIntersectionObserver.getLastOrFail();
    act(() => { observer.trigger(container, true); });
  ...
  it("calls stop() when element intersects with stop action and animation is playing", () => {
    ...
    renderHook(() => { useScrollMode(target, stopConfig, true); });
    const observer = MockIntersectionObserver.getLastOrFail();
    act(() => { observer.trigger(container, true); });
  ...
  it("removes scroll listener when element leaves viewport", () => {
    ...
    const observer = MockIntersectionObserver.getLastOrFail();
    act(() => { observer.trigger(container, true); });   // enter
    act(() => { observer.trigger(container, false); });  // leave
  ...
  it("calls goToAndPlay(0) for playOnce action without frames", () => {
    ...
    renderHook(() => { useScrollMode(target, config, true); });
    const observer = MockIntersectionObserver.getLastOrFail();
    act(() => { observer.trigger(container, true); });
  ...
  it("calls playSegments for playOnce action with frames", () => {
    const config: ScrollConfig = {
      mode: InteractivityMode.scroll,
      actions: [{ visibility: [0, 1], type: InteractivityActionType.playOnce, frames: [10, 50] }],
    };
    renderHook(() => { useScrollMode(target, config, true); });
    const observer = MockIntersectionObserver.getLastOrFail();
    act(() => { observer.trigger(container, true); });
  ...
  it("does not re-fire playOnce after it already played", () => {
    ...
    renderHook(() => { useScrollMode(target, config, true); });
    const observer = MockIntersectionObserver.getLastOrFail();
    act(() => { observer.trigger(container, true); }); // first intersection
  ...
  it("calls playSegments for playSegments action type", () => {
    const config: ScrollConfig = {
      mode: InteractivityMode.scroll,
      actions: [{ visibility: [0, 1], type: InteractivityActionType.playSegments, frames: [0, 50] }],
    };
    renderHook(() => { useScrollMode(target, config, true); });
    const observer = MockIntersectionObserver.getLastOrFail();
    act(() => { observer.trigger(container, true); });
